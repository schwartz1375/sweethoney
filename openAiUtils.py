...
def getOpenAiResults(pe):
    ...
    try:
        ...
        result = "The file 'apikey.txt' was not found."
        return result
    ...
    iat = extract_iat(pe)
    print(
        f"[+] A max of {len(iat)} functions will be requested to OpenAI if not cached!")

    # Estimate the cost of the requests
    estimated_cost = len(iat) * 0.002
    print(f"[!] Estimated cost of requests: ${estimated_cost}\n")
    user_input = input("Do you wish to continue? (y/n): ")
    if user_input.lower() != 'y':
        result = "Operation cancelled by the user."
        return result
